...
module.exports = async (req, res) => {
  ...
  if (!title || !content) return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.NULL_VALUE));
  ...
  try {
    ...
    const postPost = await postDB.postPost(client, user_id, title, content);
    
    // 성공적으로 posts를 가져왔다면, response를 보내줍니다.
    res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.ADD_ONE_POST_SUCCESS, postPost));

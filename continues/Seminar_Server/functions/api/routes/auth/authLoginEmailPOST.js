...
module.exports = async(req, res) => {
    ...
    try {
        ...
        const userFirebase = await signInWithEmailAndPassword(firebaseAuth, email, password) // Firebase Authentication을 통해 유저 인증
        .then((user) => user)
        .catch((e) => {
            console.log(e);
        ...
        const user = await userDB.getUserByIDFirebase(client, idFirebase); // RDS DB에 저장된 user 조회
        const {accesstoken} = jwtHandlers.sign(user); // JWT 발급
        res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.LOGIN_SUCCESS, {user, accesstoken})); // user + JWT를 response로 전송
    } catch (error) {
        console.log(error);

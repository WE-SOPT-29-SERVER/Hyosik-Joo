...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const updatePost = await postDB.updatePost(client, postId, title, content);
    if (!updatePost) return res.status(statusCode.NOT_FOUND).send(util.fail(statusCode.NOT_FOUND, responseMessage.NO_POST));
    
    // 성공적으로 posts를 가져왔다면, response를 보내줍니다.
    res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.UPDATE_ONE_POST_SUCCESS, updatePost));
